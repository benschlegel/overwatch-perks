...
import PerkCardPersonal from '@/app/cheatsheet/components/personal-perk-card';
...
export default function PersonalCardWrapper({ heroId }: Props) {
	const perks = useMemo(() => {
	...
	return (
		<div className="flex flex-col w-full sm:gap-3 gap-2 sm:mt-2 mt-1">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 px-2 sm:px-0 gap-y-4 sm:gap-x-3">
				<PerkCardPersonal perk={perks[0]} heroId={heroId} />
				<PerkCardPersonal perk={perks[1]} heroId={heroId} />
			</div>
			<div className="flex items-center my-2">
				<Separator className="flex-1 mx-5" />
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 px-2 sm:px-0 gap-y-4 sm:gap-x-3">
				<PerkCardPersonal perk={perks[2]} heroId={heroId} />
				<PerkCardPersonal perk={perks[3]} heroId={heroId} />
			</div>
		</div>
	);
}
