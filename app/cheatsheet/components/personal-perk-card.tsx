...
export default function PerkCardPersonal({ className, perk, heroId, ...props }: Props) {
	...
	useEffect(() => {
		if (typeof window !== 'undefined' && dialog === 'none') {
			const localStorageKey = `vote-${heroId}-${perk.perkType}`;
			const pref = localStorage.getItem(localStorageKey);
			if (pref !== null) {
				setPreference(Number(pref));
			}
		}
	}, [heroId, perk.perkType, dialog]);
	if (perk === undefined) return <></>;
	// TODO: convert to grid and have 4 row layout on mobile
	return (
		<Card
			data-preference={tab === 'personal' && preference !== null && preference === perk.perkIndex}
			className={cn(
				'w-full outline-background-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-secondary data-[preference=true]:ring-1 data-[preference=true]:ring-primary-foreground/80 data-[preference=true]:ring-offset-transparent data-[preference=true]:ring-offset-1',
				className
			)}
			tabIndex={0}
			onClick={onClick}
			...
			<CardContent className="relative flex items-center justify-center flex-col h-full text-center sm:gap-2 gap-1 sm:p-4 p-2 transition-colors">
				...
				{tab === 'personal' &&
					(preference === null ? (
						<p className="text-xs text-muted-foreground">Click to vote</p>
					) : (
						perk.perkIndex === preference && <p className="text-xs text-muted-foreground">You prefer this perk</p>
					))}
			</CardContent>
		</Card>
	);
