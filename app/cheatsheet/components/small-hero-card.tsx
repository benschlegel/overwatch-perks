...
export default function SmallHeroCard({ hero }: Props) {
	...
	const [tab, _] = useTabParam();
	const queryParam = tab !== 'info' ? `?tab=${tab}` : '';
	return (
		<Link href={`/cheatsheet/${hero.id}${queryParam}`} className="transform transition-transform hover:scale-125 hover:z-[9999] overflow-visible">
